...
import {
  Box, Container, Button, Alert, Grid, TextField, MenuItem, Typography
} from '@mui/material';
...
export default function EditarReservaPage() {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        const res = await fetch(`http://localhost:3000/reserva/${id}`);
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3000/reserva/editar', {
        method: 'PUT',
  ...
  return (
    <Container sx={{ py: 4 }}>
      <Box component="form" onSubmit={handleSalvar}>
        <Typography variant="h5" gutterBottom>Editar Reserva #{id}</Typography>
